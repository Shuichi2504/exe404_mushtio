(function () {
  ...
  function statusText(kind, value) {
    ...
    if (kind === "humidity") {
      if (value < 70) return "Thấp hơn ngưỡng";
      if (value > 96) return "Vượt ngưỡng";
      return "Trong ngưỡng tốt";
    }
    if (kind === "airQuality") {
    ...
    if (kind === "soilMoisture") {
      if (value < 30) return "Thấp hơn ngưỡng";
      if (value > 80) return "Vượt ngưỡng";
      return "Trong ngưỡng tốt";
    }
    return "Trong ngưỡng tốt";
  }
  ...
  function activeAlerts() {
    ...
    sensorKeys().forEach(function (key) {
      var sensor = state.sensors[key];
      var name = sensorName(key);
      var temp = metricValue(sensor, "temperature");
      var hum = metricValue(sensor, "humidity");
      var air = metricValue(sensor, "airQuality");
      var soil = metricValue(sensor, "soilMoisture");

      if (temp !== null && temp > 35) alerts.push(name + ": nhiệt độ khẩn cấp " + formatValue(temp, "°C"));
      else if (temp !== null && temp > 30) alerts.push(name + ": nhiệt độ cao " + formatValue(temp, "°C"));
      else if (temp !== null && temp < 16) alerts.push(name + ": nhiệt độ quá thấp " + formatValue(temp, "°C"));

      if (hum !== null && hum < 70) alerts.push(name + ": độ ẩm không khí thấp " + formatValue(hum, "%"));
      else if (hum !== null && hum > 96) alerts.push(name + ": độ ẩm không khí vượt ngưỡng " + formatValue(hum, "%"));

      if (air !== null && air > 1000) alerts.push(name + ": chất lượng không khí rất kém " + formatValue(air, "ppm"));
      if (soil !== null && soil < 30) alerts.push(name + ": độ ẩm đất thấp " + formatValue(soil, "%"));
    });
    return alerts;
  }
  ...
  function alertDialog() {
    ...
    var signature = alerts.join("|");
    if (!alerts.length || readSession("mushtio.dismissedAlert") === signature) return "";
    return '<div class="alert-overlay"><section class="alert-modal card pad"><div class="row"><div class="bubble" style="background:var(--danger)">!</div><div><h2>Cảnh báo môi trường</h2><p class="muted">Có thông số vượt ngưỡng an toàn.</p></div></div><ul>' +
      alerts.map(function (item) { return "<li>" + escapeHtml(item) + "</li>"; }).join("") +
      '</ul><div class="actions"><button class="btn primary" data-action="dismiss-alert" data-alert-signature="' + escapeHtml(signature) + '">Đã hiểu</button></div></section></div>';
  }
  ...
  function shell(content) {
    return '' +
      '<div class="app-shell">' +
      '<aside class="sidebar"><div class="brand"><div class="brand-mark">M</div><span>Mushtio</span></div><nav class="nav">' + navMarkup() + '</nav><div class="sidebar-footer">Phiên đăng nhập được bảo vệ. Đăng xuất khi dùng xong.</div></aside>' +
      '<main class="content">' + content + '</main>' +
      '<nav class="mobile-nav">' + navMarkup() + '</nav>' +
      '</div>' +
      detailDrawer() + alertDialog();
  }
  ...
  function logbookView() {
    return topbar("Logbook điện tử", "Giá trị thấp nhất và cao nhất theo giờ") +
      '<section class="section"><div class="actions" style="justify-content:flex-start"><button class="btn primary" data-action="generate-logbook">Tạo lại hôm nay</button><button class="btn" data-action="download-csv">Tải CSV</button></div></section>' +
    ...
    document.querySelectorAll("[data-action]").forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        if (action === "dismiss-alert") {
          writeSession("mushtio.dismissedAlert", button.getAttribute("data-alert-signature") || "");
          render();
        }
      });
    });
  ...
  function logout() {
    removeStorage(TOKEN_KEY);
    removeSession(TOKEN_KEY);
    removeSession("mushtio.dismissedAlert");
    state.token = "";
    state.user = null;
    render();
  }
  ...
  function loadSensors() {
    ...
    return Promise.resolve(assigned.map(function (item) { return item.deviceKey; })).then(function (keys) {
  ...
  function downloadCsv() {
    fetch(state.apiBase + "/logbooks/today/csv", {
      ...
      state.error = error.message || "Không tải được CSV";
      render();
    });
  }
